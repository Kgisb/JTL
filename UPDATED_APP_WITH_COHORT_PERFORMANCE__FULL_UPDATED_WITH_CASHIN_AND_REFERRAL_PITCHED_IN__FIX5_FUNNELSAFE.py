...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel","Lead Movement","Stuck deals","Deal Velocity","Deal Decay","Carry Forward", "Referral Pitched In", "Closed Lost"],
...
def _render_funnel_closed_lost(
    df_f,
    create_col: str | None,
    closed_lost_col: str | None,
    counsellor_col: str | None,
    country_col: str | None,
    source_col: str | None,
):
    ...
    st.subheader("Funnel & Movement — Closed Lost (trend)")

    # Column checks
    if not closed_lost_col or closed_lost_col not in df_f.columns:
        st.warning("Closed Lost Trigger Date column not found.", icon="⚠️")
        return
    if not create_col or create_col not in df_f.columns:
        st.warning("Create Date column not found.", icon="⚠️")
        return
    ...
    with c2:
        grain = st.radio("Time grain", ["Daily","Weekly","Monthly"], index=2, horizontal=True, key="cl_grain")

    preset = st.radio("Date scope", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="cl_scope")

    # Grouping controls
    group_options = {
        "Academic Counsellor": counsellor_col,
        "Country": country_col,
        "JetLearn Deal Source": source_col,
    }
    # Allow multiple fields to combine into one composite group label
    group_fields = st.multiselect(
        "Breakdown by (pick one or more)",
        options=list(group_options.keys()),
        default=[],
        help="Leave empty for Overall. Choose multiple to group by their combination."
    )
    show_overall = st.toggle("Also show Overall series", value=bool(group_fields), key="cl_show_overall")

    # Chart options
    colA, colB, colC = st.columns([1,1,1])
    with colA:
        chart_type = st.selectbox("Chart type", ["Line","Area","Bar"], index=0, key="cl_chart_type")
    with colB:
        normalize = st.toggle("Normalize to % share", value=False, help="Each date bucket sums to 100% across groups.", key="cl_norm")
        cumulative = st.toggle("Cumulative", value=False, help="Cumulative count/share over time per group.", key="cl_cum")
    with colC:
        smooth = st.toggle("Smooth (rolling mean)", value=False, key="cl_smooth")
        default_win = 7 if chart_type != "Monthly" else 3
        win = st.slider("Window", min_value=2, max_value=28, value=7, step=1, help="Rolling window size (buckets).", key="cl_win") if smooth else 0

    # Top-N groups
    if group_fields:
        topN = st.number_input("Top-N groups to display", min_value=1, max_value=50, value=8, step=1, key="cl_topN")
    else:
        topN = None
    ...
    if preset == "Today":
        ...
        c3, c4 = st.columns(2)
        with c3: start_d = st.date_input("Start", value=month_bounds(today)[0], key="cl_start")
        with c4: end_d   = st.date_input("End", value=today, key="cl_end")
        if end_d < start_d:
            start_d, end_d = end_d, start_d
    ...
    _C = _to_dt_date(df_f[create_col])
    _CL = _to_dt_date(df_f[closed_lost_col])

    # Window masks
    cl_in = (_CL >= start_d) & (_CL <= end_d)
    if mode == "MTD":
        create_in = (_C >= start_d) & (_C <= end_d)
        in_scope = cl_in & create_in
    else:
        in_scope = cl_in

    d = df_f.loc[in_scope].copy()
    if d.empty:
        st.info("No Closed Lost rows in the selected window.")
        return
    ...
    if not group_fields:
        ...
        for gf in group_fields:
            colname = group_options.get(gf)
            if colname and colname in d.columns:
                parts.append(d[colname].fillna("Unknown").astype(str))
        if parts:
            d["_grp"] = parts[0]
            for p in parts[1:]:
                d["_grp"] = d["_grp"] + " • " + p
    ...
    # Date bucket
    ts = pd.to_datetime(_CL, errors="coerce")
    if grain == "Daily":
        d["_bucket"] = ts.dt.to_period("D").dt.to_timestamp().dt.date
    elif grain == "Weekly":
        d["_bucket"] = (ts - pd.to_timedelta(ts.dt.weekday, unit="D")).dt.date
    else:
        d["_bucket"] = ts.dt.to_period("M").dt.to_timestamp().dt.date

    # Aggregate counts
    agg = d.groupby(["_bucket","_grp"], dropna=False).size().rename("Count").reset_index()

    # Top-N handling
    if topN is not None:
        totals = agg.groupby("_grp")["Count"].sum().sort_values(ascending=False)
        keep = list(totals.head(int(topN)).index)
        agg["_grp"] = np.where(agg["_grp"].isin(keep), agg["_grp"], "Other")

        # re-aggregate after bucketing others
        agg = agg.groupby(["_bucket","_grp"], dropna=False)["Count"].sum().reset_index()

    # Normalize to % per bucket
    if normalize:
        bucket_sum = agg.groupby("_bucket")["Count"].transform("sum").replace(0, np.nan)
        agg["Value"] = (agg["Count"] / bucket_sum) * 100.0
        y_field = "Value:Q"
        y_title = "% of bucket"
    else:
        agg["Value"] = agg["Count"]
    ...
    # Smooth or cumulative within each group, ordered by bucket
    agg = agg.sort_values(["_grp","_bucket"])
    if smooth and win and win >= 2:
        agg["Value"] = agg.groupby("_grp")["Value"].transform(lambda s: s.rolling(int(win), min_periods=1).mean())
    if cumulative:
        agg["Value"] = agg.groupby("_grp")["Value"].transform("cumsum")

    # Optional overall overlay when breakdown used
    if show_overall and group_fields:
        tot = d.groupby("_bucket").size().rename("Count").reset_index()
        if normalize:
            bsum = tot["Count"].sum()
            # For overall series under normalize: compute share vs max per-bucket (set as 100) -> better show raw normalized by its own max
            # Simpler: convert to percentage of bucket as 100 (since single group), equals 100 always.
            # But that would flatten line; skip adding overall under normalize.
            pass
        else:
            tot["Value"] = tot["Count"]
            tot["_grp"] = "Overall"
            agg = pd.concat([agg, tot[["_bucket","_grp","Value"]]], ignore_index=True)

    # Chart type
    mark = {"Line":"line","Area":"area","Bar":"bar"}[chart_type]
    base = alt.Chart(agg)
    if mark == "line":
    ...
    sel = alt.selection_point(fields=["_grp"], bind="legend")
    chart = chart.encode(
        x=alt.X("_bucket:T", title="Date"),
        y=alt.Y(y_field, title=y_title),
        color=alt.Color("_grp:N", title="Group"),
        opacity=alt.condition(sel, alt.value(1), alt.value(0.25)),
        tooltip=[alt.Tooltip("_bucket:T", title="Date"),
                 alt.Tooltip("_grp:N", title="Group"),
                 alt.Tooltip("Value:Q", format=".1f" if normalize else ".0f", title="Value")]
    ).add_params(sel).properties(height=380)

    st.caption(f"Window: **{start_d} → {end_d}** • Mode: **{mode}** • Grain: **{grain}** • Breakdown: **{', '.join(group_fields) if group_fields else 'Overall'}**")
    st.altair_chart(chart, use_container_width=True)

    # Table
    table = agg.sort_values(["_bucket","_grp"]).rename(columns={"_bucket":"Date","_grp":"Group","Value":"Value"})
    if not normalize:
        # Also include raw Count when not normalized
        table = table.merge(d.groupby(["_bucket","_grp"], dropna=False).size().rename("Count").reset_index(),
                            left_on=["Date","Group"], right_on=["_bucket","_grp"], how="left").drop(columns=["_bucket","_grp"])
    st.dataframe(table, use_container_width=True, hide_index=True)
    st.download_button(
        "Download CSV — Closed Lost trend",
        table.to_csv(index=False).encode("utf-8"),
        file_name="funnel_closed_lost_trend.csv",
        mime="text/csv",
        key="cl_trend_dl"
    )
...
# =========================
    ...
    def _carry_forward_tab():
        ...
        _create = create_col if ("create_col" in globals() and create_col in df_f.columns) else find_col(df_f, ["Create Date","Created Date","Deal Create Date","CreateDate","Created On","Creation Date","Deal Created Date","Create_Date"])
        _pay = pay_col if ("pay_col" in globals() and pay_col in df_f.columns) else find_col(df_f, ["Payment Received Date","Payment Date","Enrolment Date","PaymentReceivedDate","Enrollment Date","Enroll Date"])

        # Optional filters
        _cty = country_col if ("country_col" in globals() and country_col in df_f.columns) else find_col(df_f, ["Country","Student Country","Deal Country"])
        _src = source_col if ("source_col" in globals() and source_col in df_f.columns) else find_col(df_f, ["JetLearn Deal Source","Deal Source","Source"])
        _cns = counsellor_col if ("counsellor_col" in globals() and counsellor_col in df_f.columns) else find_col(df_f, ["Academic Counsellor","Academic Counselor","Counsellor","Advisor"])
...
elif view == "Closed Lost":
    _render_funnel_closed_lost(
        df_f=df_f,
        create_col=create_col,
        closed_lost_col=find_col(df, [
            "[Deal Stage] - Closed Lost Trigger Date",
            "Deal Stage - Closed Lost Trigger Date",
            "Closed Lost Trigger Date",
            "Closed_Lost_Trigger_Date"
        ]),
        counsellor_col=counsellor_col,
...
if view == "Funnel":
    def _funnel_tab():
        ...
        _create = create_col if ("create_col" in globals() and create_col in df_f.columns) else find_col(df_f, ["Create Date","Created Date","Deal Create Date","CreateDate","Created On","Creation Date","Deal Created Date","Create_Date"])
        _pay = pay_col if ("pay_col" in globals() and pay_col in df_f.columns) else find_col(df_f, ["Payment Received Date","Payment Date","Enrolment Date","PaymentReceivedDate","Enrollment Date","Enroll Date"])
        _first = first_cal_sched_col if ("first_cal_sched_col" in globals() and first_cal_sched_col in df_f.columns) else find_col(df_f, ["First Calibration Scheduled Date","First Calibration","First Cal Scheduled","First_Calibration_Scheduled_Date"])
        _resch = cal_resched_col if ("cal_resched_col" in globals() and cal_resched_col in df_f.columns) else find_col(df_f, [
            "Calibration Rescheduled Date","Cal Rescheduled","Rescheduled Date"
        ])
        _done = cal_done_col if ("cal_done_col" in globals() and cal_done_col in df_f.columns) else find_col(df_f, ["Calibration Done Date","Calibration Done","Cal Done","Cal_Done_Date"])
