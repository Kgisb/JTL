...
def _render_funnel_closed_lost(
    ...
    group_by = st.selectbox(
        "Group by",
        ["Overall","Academic Counsellor","Country","JetLearn Deal Source"],
        index=0,
        help="Pick a breakdown; 'Overall' shows a single series.",
        key="cl_group_by"
    )
    show_overall = st.toggle("Also show Overall series", value=(group_by!="Overall"), key="cl_show_overall")
    ...
    if group_by == "Overall":
        d["_grp"] = "Overall"
    elif group_by == "Academic Counsellor" and counsellor_col and counsellor_col in d.columns:
        d["_grp"] = d[counsellor_col].fillna("Unknown").astype(str)
    elif group_by == "Country" and country_col and country_col in d.columns:
        d["_grp"] = d[country_col].fillna("Unknown").astype(str)
    elif group_by == "JetLearn Deal Source" and source_col and source_col in d.columns:
    ...
    # Date bucket
    if grain == "Daily":
        d["_bucket"] = pd.to_datetime(_CL, errors="coerce").dt.to_period("D").dt.to_timestamp().dt.date
    elif grain == "Weekly":
        ...
        d["_bucket"] = pd.to_datetime(_CL, errors="coerce").dt.to_period("M").dt.to_timestamp().dt.date

    # Aggregate
    agg = d.groupby(["_bucket","_grp"], dropna=False).size().rename("Count").reset_index()

    # Optional overall overlay
    if show_overall and group_by != "Overall":
        total = d.groupby("_bucket").size().rename("Count").reset_index()
        total["_grp"] = "Overall"
        agg = pd.concat([agg, total], ignore_index=True)

    # Chart
    st.caption(f"Window: **{start_d} → {end_d}** • Mode: **{mode}** • Grain: **{grain}** • Group: **{group_by}**")
    chart = (
        alt.Chart(agg)
        ...
        .encode(
            ...
            tooltip=[alt.Tooltip("_bucket:T", title="Date"), alt.Tooltip("_grp:N", title="Group"), alt.Tooltip("Count:Q")]
        )
    ...
    # Table
    agg_sorted = agg.sort_values(["_bucket","_grp"]).rename(columns={"_bucket":"Date","_grp":"Group"})
