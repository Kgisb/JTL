...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel","Lead Movement","Stuck deals","Deal Velocity","Deal Decay","Carry Forward", "Referral Pitched In"],
...
def _classify_interval(row, cols):
    ...
    # If CL is before the first milestone
...
def _render_funnel_closed_lost_analysis(
    ...
    # Hard requirement: Closed Lost Trigger Date + Create Date for MTD logic
    if _cl_date is None:
    ...
    # ---- Output A: Closed Lost by dimension(s)
    st.markdown("### A) Closed Lost — Counts by Dimension")
    # Map UI dims to internal columns
    def _map_dim(name):
    ...
    # Chart
    if chart_type == "Stacked Bar":
        # If 1 dim → simple bars; if 2 dims → stack by second
        if len(dim_cols) == 1:
        ...
        # Line: daily time series within window; color by first dim if present
        ...
        ch_a = _alt.Chart(ts).mark_line(point=True).encode(
            ...
            y=_alt.Y("Closed Lost Count:Q"),
            color=color_enc,
            tooltip=[_alt.Tooltip("_d:T", title="Date"), _alt.Tooltip("Closed Lost Count:Q")] + (
    ...
    # ---- Output B: Journey interval analysis
    ...
    # Compute interval bucket + days_to_loss
    dd["_interval"], dd["_days_to_loss"] = zip(*dd.apply(lambda r: _classify_interval(r, cols_map), axis=1))

    # Aggregate by interval (+ optional first chosen dim)
    by_cols = ["_interval"]
    label_interval = "Interval Bucket"
    if dim_cols:
        by_cols.append(dim_cols[0])
    # robust groupby size
    ...
    # Bubble chart
    if dim_cols:
        color_enc = _alt.Color(f"{dim_cols[0]}:N", title=(sel_dims[0]))
    else:
        color_enc = _alt.value(None)
    ...
    pretty_cols = {}
    if dim_cols:
        pretty_cols[dim_cols[0]] = sel_dims[0]
...
# ------- Wire into NAV + Router safely (non-destructive) -------
try:
    # Add pill to Funnel & Movement if missing
...
# Call router when selected
try:
    if view == "Closed Lost Analysis":
        # Use existing column helpers if present, else None
        ...
        _cl_hint  = None  # let resolver find from candidates
