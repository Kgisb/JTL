...
def _render_funnel_booking_analysis(
    ...
    # Derived presence flags for extra slice-by options (Yes/No)
    _first_col  = _first
    ...
    d["_HAS_FIRST"]   = _pd.Series(["No"]*len(d))
    d["_HAS_RESCH"]   = _pd.Series(["No"]*len(d))
    d["_HAS_CALDONE"] = _pd.Series(["No"]*len(d))
    d["_HAS_ENRL"]    = _pd.Series(["No"]*len(d))

    if _first_col:
        d["_HAS_FIRST"] = _pd.Series(_pd.notna(d[_first_col]).map({True:"Yes", False:"No"}))
    if _resch_col:
        d["_HAS_RESCH"] = _pd.Series(_pd.notna(d[_resch_col]).map({True:"Yes", False:"No"}))
    if _caldone_col:
        d["_HAS_CALDONE"] = _pd.Series(_pd.notna(d[_caldone_col]).map({True:"Yes", False:"No"}))
    if _enrol_col:
        # Enrollment sometimes stored as strings; coerce to datetime to detect presence too
        _enrl_dt = _bk_to_dt(d[_enrol_col])
        d["_HAS_ENRL"] = _pd.Series(_pd.notna(_enrl_dt).map({True:"Yes", False:"No"}))
    ...
    def _map_dim(x):
        return {
            "Academic Counsellor":"_AC",
            "Country":"_CNT",
            "JetLearn Deal Source":"_SRC",
            "Booking Type":"_BKTYPE",
            "First Trial":"_HAS_FIRST",
            "Trial Reschedule":"_HAS_RESCH",
            "Calibration Done":"_HAS_CALDONE",
            "Enrolment":"_HAS_ENRL",
        }.get(x)
