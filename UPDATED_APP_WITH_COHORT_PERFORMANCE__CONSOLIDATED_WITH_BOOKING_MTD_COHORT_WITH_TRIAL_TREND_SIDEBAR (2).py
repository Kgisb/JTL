...
def _render_funnel_closed_lost_analysis(df_f, counsellor_col_hint=None, country_col_hint=None, source_col_hint=None,
    ...
    if chart_type == "Stacked Bar":
        ...
        # Horizontal bars: time on Y and counts on X
...
def _render_funnel_booking_analysis(df_f, trigger_book_col_hint=None, cal_slot_col_hint=None, first_cal_sched_col_hint=None,
    ...
    if scope == "Today":
        ...
        ch = _alt.Chart(series).mark_bar().encode(
            y=y_enc,
            x=_alt.X("Count:Q"),
            color=color_enc,
...
def _render_funnel_trial_trend(
    ...
    # Chart selector
    chart_type = st.radio("Chart", ["Stacked Bar","Horizontal Bar","Line"], index=0, horizontal=True, key="tt_chart")

    if chart_type == "Stacked Bar":
# (horizontal branch will be inserted later)
        # Horizontal bars: time on Y and counts on X
        y_enc = _alt.Y("_day:T", title=None) if gran == "Daily" else _alt.Y("_mon:T", title=None)
        color_enc = _alt.Color("Metric:N", title="Metric")
        ch = _alt.Chart(ser).mark_bar().encode(
