...
def _render_funnel_closed_lost_analysis(df_f, counsellor_col_hint=None, country_col_hint=None, source_col_hint=None,
    ...
    with c1:
        mode = st.radio("Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key="cl_mode_v2")
    with c2:
        scope = st.radio("Date scope", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="cl_scope_v2")
    with c3:
        chart_type = st.radio("Chart", ["Stacked Bar","Line"], index=0, horizontal=True, key="cl_chart_v2")
    with c4:
        dim_opts = []
        ...
        sel_dims = st.multiselect("Dimensions (choose 1–2 for best visuals)", options=dim_opts, default=(dim_opts[:1] if dim_opts else []), key="cl_dims_v2")
    ...
    if scope == "Today":
        ...
        with cc1: start_d = st.date_input("Start", value=_month_bounds(today)[0], key="cl_start_v2")
        with cc2: end_d   = st.date_input("End", value=today, key="cl_end_v2")
        if end_d < start_d:
            start_d, end_d = end_d, start_d
...
# Trial Trend placeholder (will be replaced by full impl)
def _render_funnel_trial_trend(df, *args, **kwargs):
    import streamlit as st
    st.error('Trial Trend placeholder — upload shows rendering hook works. Send error to fix.')
...
# ---- Full Trial Trend implementation (appended) ----
def _render_funnel_trial_trend(
    df_f,
    create_col_hint=None,
    counsellor_col_hint=None,
    country_col_hint=None,
    source_col_hint=None,
    ...
    with c0: mode = st.radio("Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key="tt_mode_v2")
    with c1:
        today=_date.today()
        scope = st.radio("Date scope", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="tt_scope_v2")
        if scope=="Today": start_d,end_d=today,today
        elif scope=="Yesterday": start_d=today-_timedelta(days=1); end_d=start_d
        elif scope=="This Month":
            start_d=_date(today.year,today.month,1); end_d=today
        elif scope=="Last Month":
            ...
            a,b=st.columns(2)
            with a: start_d=st.date_input("Start", value=_date(today.year,today.month,1), key="tt_start_v2")
            with b: end_d=st.date_input("End", value=today, key="tt_end_v2")
            if end_d<start_d: start_d,end_d=end_d,start_d
    with c2: gran = st.radio("Granularity", ["Daily","Monthly"], index=0, horizontal=True, key="tt_gran_v2")
    with c3:
        dims=st.multiselect("Slice by", options=["Academic Counsellor","Country","JetLearn Deal Source"], default=["Academic Counsellor"], key="tt_dims_v2")
    ...
    with fc1:
        ...
        sel_ac=st.multiselect("Academic Counsellor", ac_opts, default=["All"], key="tt_ac_v2")
    with fc2:
        ctry_opts=["All"]+sorted(ev["_CNT"].unique().tolist())
        sel_cty=st.multiselect("Country", ctry_opts, default=["All"], key="tt_cty_v2")
    with fc3:
        src_opts=["All"]+sorted(ev["_SRC"].unique().tolist())
        sel_src=st.multiselect("JetLearn Deal Source", src_opts, default=["All"], key="tt_src_v2")
    ...
    chart = st.radio("Chart", ["Stacked Bar","Horizontal Bar","Line"], index=0, horizontal=True, key="tt_chart_v2")
    ...
    st.download_button("Download CSV — Trial Trend", data=pretty.to_csv(index=False).encode("utf-8"),
                       file_name="trial_trend.csv", mime="text/csv", key="tt_dl_tbl_v2")

    st.markdown("### % Trend (B / A)")
    metric_opts=["Lead","Trial","Trial Done","Enrollment"]
    den=st.selectbox("Metric A (denominator)", metric_opts, index=1, key="tt_den_v2")
    num=st.selectbox("Metric B (numerator)", metric_opts, index=2, key="tt_num_v2")
    ...
    if den in piv.columns and num in piv.columns:
        ...
        st.download_button("Download CSV — % Trend (B over A)", data=pretty_rate.to_csv(index=False).encode("utf-8"),
                           file_name="trial_percentage_trend.csv", mime="text/csv", key="tt_dl_rate_v2")
    else:
        st.info("Selected metrics have no data in the window.")
# ---- /Full Trial Trend implementation ----
